...
Want to run Phase 1 from Slack instead of the terminal? There's a small
Bolt app for that under `slack_integration/` / `mom_phase1/slack_app.py`.
It needs a couple of extra libraries the core package doesn't:

```bash
pip install -r slack_integration/requirements.txt
```

(Using pipx instead? Inject them into the same venv pipx built for
`mom-phase1`:)
```bash
pipx inject mom-phase1 slack_bolt requests python-dotenv
```

Then drop your Slack tokens into a `.env` file in the repo root (already
gitignored):
```
...
If you installed with `pip`/venv, that's it. If you installed with **pipx**,
run it with that venv's own Python instead (this is what actually has
`slack_bolt` etc. installed):
```bash
"$(pipx environment --value PIPX_LOCAL_VENVS)/mom-phase1/bin/python" mom_phase1/slack_app.py
```
(One-time convenience: add an alias to your shell profile so you don't have
to type that path every time —
`alias mom-slack='"$(pipx environment --value PIPX_LOCAL_VENVS)/mom-phase1/bin/python" mom_phase1/slack_app.py'`
— then just run `mom-slack` from inside `note_taker/`.)

Full setup (creating the Slack app, scopes, slash commands) is in
`slack_integration/README.md`.